...
import type { BookingHistory } from "../../utils/types";
...
export default function HistoryTab() {
  ...
  const [historyData, setHistoryData] = useState<BookingHistory[]>([]);
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        const res = await api.get<{ data: BookingHistory[] }>(
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-gray-500">
            {t("dashboard.user.history.subtitle")}
          </p>
        </div>

        <div className="text-right text-sm">
          <p className="text-gray-500">
            {t("dashboard.user.history.total_orders")}
            <span className="font-semibold text-black">{totalOrders}</span>
          </p>
          <p className="text-gray-500">
            {t("dashboard.user.history.total_spent")}
          ...
          <thead className="bg-gray-200">
            <tr>
              <th className="px-4 py-3 text-left font-semibold">
                {t("dashboard.user.history.table.service")}
              </th>
              <th className="px-4 py-3 text-left font-semibold">
                {t("dashboard.user.history.table.provider")}
              </th>
              <th className="px-4 py-3 text-left font-semibold">
                {t("dashboard.user.history.table.date")}
              </th>
          ...
          <tbody>
            {currentData.map((item, index) => (
              <tr key={index} className="border-t hover:bg-gray-50 transition">
                ...
                <td className="px-4 py-3">{item.provider.display_name}</td>
